...
from config_parser import Belt, Sensor, SimulatorConfig, TopicConfig, load_config
...
def _find_component(config: SimulatorConfig, component_id: str):
    for collection in [config.belts, config.sensors, config.sources, config.sinks]:
