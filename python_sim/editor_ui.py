...
from config_parser import Belt, Sensor, Source, Sink, TopicComponent, TopicConfig
...
class EditorUI:
    def __init__(self, manager: pygame_gui.UIManager, width: int, height: int, config=None):
        ...
        self.toolbar_panel = pygame_gui.elements.UIPanel(
            relative_rect=pygame.Rect(10, 50, 120, 280),
            manager=self.manager
        )
    ...
    def open_manager_panel(self):
        ...
        self.manager_window = pygame_gui.elements.UIWindow(
            rect=pygame.Rect(150, 70, 430, 520),
            manager=self.manager,
        ...
        self.manager_details = pygame_gui.elements.UITextBox(
            ...
            relative_rect=pygame.Rect(200, 10, 205, 395),
            manager=self.manager,
            container=container
        )
    ...
    def _make_unique_id(self, kind: str, base_id: str) -> str:
        existing = {
            item.id
            for collection in [self.config.belts, self.config.sensors, self.config.sources, self.config.sinks]
            for item in collection
        }
        base = f"{base_id}_copy"
        candidate = base
        suffix = 2
    ...
    def _component_options(self):
        options = []
        for kind, items in [("belt", self.config.belts), ("sensor", self.config.sensors), ("source", self.config.sources), ("sink", self.config.sinks)]:
            for item in items:
    ...
    def _topic_component_options(self):
        options = []
        for kind, items in [("belt", self.config.belts), ("sensor", self.config.sensors)]:
            for item in items:
                options.append(f"{kind}:{item.id}")
        return options or ["none"]
    ...
    def _add_component_to_topic(self):
        ...
        if not topic or not item or kind not in {"belt", "sensor"}:
            return
    ...
    def handle_event(self, event) -> str:
        ...
        if event.type == pygame_gui.UI_BUTTON_PRESSED:
            if event.ui_element == self.save_btn:
                ...
                self.refresh_manager_panel()
            # elif event.ui_element == self.manager_clear_btn:
